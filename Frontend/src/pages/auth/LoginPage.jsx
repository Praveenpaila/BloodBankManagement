    ...
    <main className="grid min-h-screen md:grid-cols-2">
      ...
      <section className="grid place-items-center p-4">
        <form className="card w-full max-w-md" onSubmit={handleSubmit(submit)}>
          ...
          <button className="btn-primary mt-5 w-full" disabled={isSubmitting}>{isSubmitting ? 'Signing in...' : 'Login'}</button>
          <div className="mt-4 flex justify-between text-sm">
            <Link to="/forgot-password">Forgot password?</Link>
            <Link className="font-bold text-[#C0392B]" to="/register">Register</Link>
          </div>
        </form>
      </section>
    </main>
