...
export const EligibilityPage = () => {
  ...
  return (
    <DashboardLayout title="Eligibility Check" subtitle={`Step ${step}/4`}>
      {previous && <div className="card mb-5"><p className="font-black">Previous result: <span className="capitalize">{(previous.status || previous.record?.status || '').replaceAll('_', ' ')}</span></p></div>}
      <div className="card">
        <div className="mb-6 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-[#C0392B]" style={{ width: `${step * 25}%` }} /></div>
        {step === 1 && <div className="grid gap-4 md:grid-cols-2"><Field label="Age"><input className="input-field" type="number" value={form.age} onChange={(e) => update('age', e.target.value)} /></Field><Field label="Weight in kg"><input className="input-field" type="number" value={form.weight} onChange={(e) => update('weight', e.target.value)} /></Field></div>}
        ...
        {step === 4 && <div className="grid gap-4 md:grid-cols-2"><Field label="Hemoglobin"><input className="input-field" type="number" step="0.1" placeholder="e.g. 13.5" value={form.hemoglobin} onChange={(e) => update('hemoglobin', e.target.value)} /></Field><Field label="Gender"><select className="input-field" value={form.gender} onChange={(e) => update('gender', e.target.value)}><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select></Field></div>}
...
const ToggleGrid = ({ items, form, update }) => (
  <div className="grid gap-4 md:grid-cols-2">
    {items.map((item) => (
      <button key={item} type="button" className={form[item] ? 'btn-primary' : 'btn-outline'} onClick={() => update(item, !form[item])}>
        {item.replace(/([A-Z])/g, ' $1')}: {form[item] ? 'Yes' : 'No'}
      </button>
    ))}
  </div>
);
...
const NotificationsView = ({ title }) => {
  ...
  const respond = async (requestId, action) => {
    try {
      await api.put(`/blood-requests/${requestId}/respond`, { action });
  ...
  return (
    <DashboardLayout title={title}>
      ...
      <div className="space-y-3">{(data || []).map((item) => (
        <div key={item._id} className={`card ${!item.isRead ? 'bg-red-50' : ''}`}>
          ...
          {item.type === 'blood_request' && <div className="mt-3 flex gap-2"><button className="btn-primary" onClick={() => respond(item.data?.requestId, 'accept')}>Accept</button><button className="btn-outline" onClick={() => respond(item.data?.requestId, 'decline')}>Decline</button></div>}
        </div>
...
export const NearbyRequestsPage = () => {
  ...
  const { data, reload } = useApi(async () => {
    ...
    return (await api.get(`/blood-requests/nearby?lat=${coords[1]}&lng=${coords[0]}`)).data.data;
  }, [coords?.join(',')]);

  const enable = () => navigator.geolocation?.getCurrentPosition(async ({ coords: c }) => {
    ...
    <DashboardLayout title="Nearby Requests">
      {!coords?.length ? <div className="card"><p className="font-black">Enable your location to see nearby requests.</p><button className="btn-primary mt-4" onClick={enable}>Update Location</button></div> : (
        <div className="grid gap-5 lg:grid-cols-[3fr_2fr]">
          ...
          <div className="space-y-3">{(data || []).map((item) => <RequestCard key={item._id} request={item} />)}{(!data || data.length === 0) && <Empty text="No nearby open requests." />}</div>
        </div>
      )}
    </DashboardLayout>
  );
...
export const RaiseRequest = () => {
  ...
  const coords = user?.location?.coordinates || [80.648, 16.5062];
  const { data: count, reload } = useApi(async () => (await api.get(`/donors/count?bloodGroup=${form.bloodGroup}&lat=${coords[1]}&lng=${coords[0]}&radius=${form.radiusKm}`)).data.data.count, [form.bloodGroup, form.radiusKm]);
  const submit = async () => {
    try {
      const { data } = await api.post('/blood-requests', { ...form, lat: coords[1], lng: coords[0] });
      toast.success(`Alert sent to ${data.data.notifiedDonors} donors`);
      reload();
    } catch (err) {
  ...
  return (
    <DashboardLayout title="Raise Blood Request">
      <div className="card space-y-5">
        <div className="grid gap-3 md:grid-cols-4">{BLOOD_GROUPS.map((group) => <button type="button" key={group} className={form.bloodGroup === group ? 'btn-primary' : 'btn-outline'} onClick={() => setForm({ ...form, bloodGroup: group })}>{group}</button>)}</div>
        <div className="grid gap-3 md:grid-cols-3">{['normal', 'urgent', 'critical'].map((urgency) => <button type="button" key={urgency} className={form.urgency === urgency ? 'btn-primary' : 'btn-outline'} onClick={() => setForm({ ...form, urgency })}>{urgency}</button>)}</div>
        <input className="input-field" type="number" value={form.unitsNeeded} onChange={(e) => setForm({ ...form, unitsNeeded: e.target.value })} />
        <textarea className="input-field" placeholder="Notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
        <Field label={`Searching within ${form.radiusKm} km`}><input className="w-full" type="range" min="5" max="50" value={form.radiusKm} onChange={(e) => setForm({ ...form, radiusKm: e.target.value })} /></Field>
        <p className="badge-pill bg-red-50 text-red-700">~{count || 0} eligible donors found</p>
        <button className="btn-primary w-full" onClick={submit}>Send Alert to {count || 0} Nearby Donors</button>
      </div>
    </DashboardLayout>
  );
};
...
const RequestsList = ({ admin = false }) => {
  ...
  return (
    <DashboardLayout title={admin ? 'Requests Log' : 'Request Status'}>
      <PageTable headers={['Hospital', 'Blood', 'Units', 'Urgency', 'Status', 'Notified', 'Date', 'Actions']} rows={rows.map((item) => <tr key={item._id}><td>{item.requestedBy?.firstName}</td><td><BloodGroupBadge group={item.bloodGroup} size="sm" /></td><td>{item.unitsNeeded}</td><td><span className={`badge-pill ${urgencyClass(item.urgency)}`}>{item.urgency}</span></td><td>{item.status}</td><td>{item.notifiedDonors?.length || 0}</td><td>{fmtDate(item.createdAt)}</td><td>{!admin && item.status === 'responding' && <button className="btn-outline" onClick={() => setStatus(item._id, 'fulfilled')}>Mark Fulfilled</button>}</td></tr>)} />
    </DashboardLayout>
  );
};
...
export const DonorSearch = () => {
  ...
  const search = async () => {
    const { data } = await api.get(`/donors/search?bloodGroup=${query.bloodGroup}&city=${query.city}`);
...
export const ExpiryAlerts = () => {
  ...
  const week = (data || []).filter((item) => new Date(item.expiryDate) <= new Date(Date.now() + 7 * 86400000)).length;
  return (
